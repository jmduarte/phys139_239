...
def get_potential(sim, sim_obj):
    ...
    def potential(x1, x2):
        ...
        if sim == "r2":
            ...
            potential = pot_a * (bounded_dist < 1) + (bounded_dist >= 1) * (
...
class SimulationDataset(object):
    ...
    def simulate(self, ns, key=0):
        ...
        def odefunc(y, t):
            ...
            a = acceleration(y)
            return np.concatenate([y[:, dim : 2 * dim], a, 0.0 * y[:, :params]], axis=1).reshape(packed_shape)
        ...
        def make_sim(key):
            ...
            x_times = odeint(odefunc, x0.reshape(packed_shape), times, mxstep=2000).reshape(-1, *unpacked_shape)
